...
1. スプレッドシートを新規作成し、拡張機能 → Apps Script でコンテナバインドのプロジェクトを開きます。
2. **自分のWorkspaceドメインに合わせて `コード.js` を1箇所書き換えます**: `getWebAppUrl()` にある `const domain = 'bazaarjapan.com';` を自組織のドメイン(例: `example.co.jp`)に変更してください。アプリ内リンク(メニューのタイルや「← メニュー」)はこのドメイン付きURLで生成されるため、変更しないと別ドメインの環境では画面遷移が404になります。**次の手順で取り込む前に書き換えておきます**(取り込み後に書き換えた場合は再度 `clasp push` などで反映してください)。
3. **このリポジトリのコードをプロジェクトへ取り込みます**(どちらか):
   - **clasp(推奨)**: リポジトリをcloneし、手順2の書き換えをしてから、`.clasp.json` の `scriptId` を自分のプロジェクトのスクリプトIDに書き換えて `clasp push`(`appsscript.json` を上書きする確認には Yes)。
   - **手動コピー**: Apps Script エディタで(手順2を反映した)`コード.js` の内容を貼り付け、さらに**すべての `.html` ファイル**を同名のHTMLファイルとして追加し、プロジェクトの設定から `appsscript.json` を表示して内容を置き換えます。
4. Apps Script エディタから `setupLibrarySystem` を一度実行(必要なシートとヘッダーを自動作成。既存データには触れません)。初回は権限の承認を求められます。
5. デプロイ → 新しいデプロイ → 種類「ウェブアプリ」で公開(実行ユーザー: 自分 / アクセス: ドメイン内)。
6. 発行されたURLを開くとメニューが表示されます。
7. スプレッドシートを開き直し、**管理メニュー →「トリガー一括設置」** を実行すると、延滞通知(毎日9時)と返却リマインダー(毎日10時)が自動化されます。
