...
- アクセス範囲はデプロイ時に選べます(Workspaceの組織内限定/自分のみ/Googleアカウントを持つ全員)。利用者情報を扱うため、できるだけ狭い範囲での運用を推奨します。
...
3. 拡張機能 → Apps Script を開き、デプロイ → 新しいデプロイ → 種類「ウェブアプリ」で公開します。初回は権限の承認を求められます。
   - **実行ユーザー**: 自分
   - **アクセスできるユーザー**: Google Workspaceの組織で使う場合は「**(組織名)内の全員**」、個人の@gmail.comで使う場合は「**自分のみ**」または「**Googleアカウントを持つ全員**」を選びます。
   - ⚠️「Googleアカウントを持つ全員」にするとURLを知っている人は誰でも開けます。利用者の氏名・連絡先を扱うため、家族や小さなサークル内など信頼できる範囲での利用にとどめてください。
   - アプリ内リンクのドメインは実行ユーザーから**自動判定**されるため、コードの書き換えは不要です(Workspaceならドメイン付きURL、個人アカウントならそのままのURLが使われます)。
4. 発行されたURLを開くとメニューが表示されます。
5. スプレッドシートを開き直し、**管理メニュー →「トリガー一括設置」** を実行すると、延滞通知(毎日9時)と返却リマインダー(毎日10時)が自動化されます。
