...
2. **このリポジトリのコードをプロジェクトへ取り込みます**(どちらか):
   - **clasp(推奨)**: リポジトリをcloneし、`.clasp.json` の `scriptId` を自分のプロジェクトのスクリプトIDに書き換えて `clasp push`(`appsscript.json` を上書きする確認には Yes)。
   - **手動コピー**: Apps Script エディタで `コード.js` の内容を貼り付け、さらに**すべての `.html` ファイル**を同名のHTMLファイルとして追加し、プロジェクトの設定から `appsscript.json` を表示して内容を置き換えます。
3. **自分のWorkspaceドメインに合わせて1箇所書き換えます**: `コード.js` の `getWebAppUrl()` にある `const domain = 'bazaarjapan.com';` を自組織のドメイン(例: `example.co.jp`)に変更してください。アプリ内リンク(メニューのタイルや「← メニュー」)はこのドメイン付きURLで生成されるため、変更しないと別ドメインの環境では画面遷移が404になります。
