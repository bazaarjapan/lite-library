...
**Operational features (run from the spreadsheet's 管理メニュー, defined in `onOpen`):** the menu shows only day-to-day items (スキーマ検証 / 整合性チェック・修復 / 延滞リマインダー送信 / 貸出状況レポート / バックアップ / トリガー一括設置・解除); one-time items (初期セットアップ, 書籍DB移行, individual trigger install/remove) are commented out in `onOpen` — uncomment to use, and keep commented lines before the active `.addItem` chain so uncommenting stays valid syntax. The legacy バーコード生成 sheet feature was removed entirely (replaced by the labels page). `installAllTriggers`/`removeAllTriggers` manage the three time-driven triggers at once (overdue daily 9:00, return reminder daily 9:00, monthly archive on the 1st).
