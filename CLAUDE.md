...
**Operational features (run from the spreadsheet's 管理メニュー, defined in `onOpen`):** the menu shows only day-to-day items (スキーマ検証 / 整合性チェック・修復 / 延滞リマインダー送信 / 貸出状況レポート / バックアップ); one-time items (初期セットアップ, 書籍DB移行, trigger install/remove) are commented out in `onOpen` — uncomment to use. The legacy バーコード生成 sheet feature was removed entirely (replaced by the labels page).
