...
- `予約DB` — reservations (created on demand by `ensureReservationSheet_`): 予約ID, 書籍ID, 書籍名, 利用者ID, 利用者名, 予約日時, 状態 (`予約中` / `貸出済` / `取消`), 処理日時. **Reservation-key rule:** 書籍ID holds the normalized ISBN when the book has one, else the 管理番号 (so any copy of the same title fulfills the reservation); matching always checks both the copy's 管理番号 and its ISBN via `findReservationForBook_`. Integrations: returns append a hold notice + best-effort email to the first reserver; lending refuses reserved books to anyone but the first reserver and marks the reservation 貸出済 on success; renewal is refused while a reservation is active. When the sheet doesn't exist, all checks no-op.
